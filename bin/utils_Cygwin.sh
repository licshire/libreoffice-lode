...
install_pre_build_native_make()
{
    ...
    if [ ! -f "${BASE_DIR?}/packages/${make_exec_file?}" -o ! -x "${BASE_DIR?}/opt/bin/make" ] ; then
        pushd "${BASE_DIR?}/packages" > /dev/null || die "Error switching to ${BASE_DIR?}/packages"
        rm -fr "${make_exec_file?}"
        wget -O "http://dev-www.libreoffice.org/bin/cygwin/${make_exec_file?}" || die "Error download ${module?} source package"
